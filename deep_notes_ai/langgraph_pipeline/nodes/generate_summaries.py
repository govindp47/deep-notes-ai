"""
deep_notes_ai/langgraph_pipeline/nodes/generate_summaries.py

Node 8: generate_summaries

Responsibility: Generate revision-note summaries for every CONTENT node via
...
Returns: {"nodes_content": dict[str, ContentStoreItem]}  # updated with .summary
...
_NODE = "generate_summaries"
_STAGE = "Generating Summaries"


def make_generate_summaries_node(
    ...
    """
    Factory that returns a generate_summaries node bound to the given service.

    Args:
        summary_service: Configured SummaryService instance.
    ...
    def generate_summaries(state: PipelineState) -> dict:
        """
        Generate revision summaries for all CONTENT nodes.
        ...
        Returns:
            {"nodes_content": dict[str, ContentStoreItem]}  # with .summary populated

        Raises:
            SummaryGenerationError: if generation fails after fallback.
        """
        ...
        PAYLOAD_METADATA_AVERAGE_TOKENS = 50

        artifact_path = current_run_dir / "nodes_content.json"

        is_complete = True
        for expected_uuid in nodes_content:
            if not nodes_content[expected_uuid].summary:
                is_complete = False
                break

        if is_complete:
            logger.info("content summaries already generated.")
            if progress_service is not None:
                progress_service.emit_info(
                    node_name=_NODE,
                    stage=_STAGE,
                    message="Summaries restored from cache",
                )
            return {}

        logger.info(
            "Generating summaries for %d CONTENT nodes",
            len(content_payload),
        )
        ...
        content_points_txt = "\n".join(
            item.content
            for item in nodes_content.values()
        )
        ...
        try:
            updated_nodes_content = summary_service.generate(
        ...
        logger.info(
            "Summary generation complete for %d nodes",
